...
class MongodbModel {
    ...
    static async model(coll_or_model_name) {
        if(!coll_or_model_name) {
            throw new Error('collection / model name is missing');
        }
        let coll_name = _.snakeCase(coll_or_model_name);

        // const placement = this.get_placement(coll);
        // const db = await this.get_db(placement);

        let db_url = 'mongodb://localhost/';
        let db_name = 'test';
        let connectOptions = { useUnifiedTopology: true, useNewUrlParser: true };
        
        const connection = await mongodb.MongoClient.connect(db_url, connectOptions);
